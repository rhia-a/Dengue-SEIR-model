...
def model (beta0, epsilon,phi,mu,rho,gamma,sigma): 
...
weekly_I = model(beta0,epsilon,phi,mu,rho, gamma, sigma) #runs model using my initial guess

def objective_beta(scaled_beta):  #optimising beta
    beta = scaled_beta[0] * 1e-6
    weekly_I = model(beta,epsilon, phi,mu,rho,gamma,sigma)
...
def objective_epsilon(epsilon):
    ...
    weekly_I = model(beta0, epsilon[0], phi, mu, rho, gamma, sigma)
...
def objective_phi(phi):
    ...
    weekly_I = model(beta0, epsilon, phi[0], mu, rho, gamma, sigma)
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
def objective_mu(mu):
    ...
    weekly_I = model(beta0, epsilon, phi, mu[0], rho, gamma, sigma)
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
def objective_rho(rho):
    ...
    weekly_I = model(beta0, epsilon, phi, mu, rho[0], gamma, sigma)
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
def objective_gamma(gamma):
    ...
    weekly_I = model(beta0, epsilon, phi, mu, rho, gamma[0], sigma)
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
def objective_sigma(sigma):
    ...
    weekly_I = model(beta0, epsilon, phi, mu, rho, gamma, sigma[0])
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
weekly_I = model(best_beta, best_epsilon, best_phi, best_mu, best_rho, best_gamma, best_sigma)
