...
def model (beta0, epsilon,phi,mu,rho): 
...
weekly_I = model(beta0,epsilon,phi,mu,rho) #runs model using my initial guess

def objective_beta(scaled_beta):  #optimising beta
    beta = scaled_beta[0] * 1e-6
    weekly_I = model(beta,epsilon, phi,mu,rho)
...
def objective_epsilon(epsilon):
    ...
    weekly_I = model(beta0, epsilon[0], phi, mu,rho)
...
def objective_phi(phi):
    ...
    weekly_I = model(beta0, epsilon, phi[0], mu, rho)
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
def objective_mu(mu):
    ...
    weekly_I = model(beta0, epsilon, phi, mu[0], rho)
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
def objective_rho(rho):
    ...
    weekly_I = model(beta0, epsilon, phi, mu, rho[0])
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
weekly_I = model(best_beta, best_epsilon, best_phi, best_mu, best_rho)
