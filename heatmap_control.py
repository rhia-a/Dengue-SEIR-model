...
durations = range(1, 43)       # 1 to 42 days

peak_matrix = np.zeros((len(durations), len(start_days)))
total_matrix = np.zeros((len(durations), len(start_days)))

for i, duration in enumerate(durations):

    for j, start_day in enumerate(start_days):

        weekly_cases, M = model(c, start_day, duration)

        peak_matrix[i, j] = np.max(weekly_cases)
        total_matrix[i, j] = np.sum(weekly_cases)


best_peak_index = np.unravel_index(
    np.argmin(peak_matrix),
    peak_matrix.shape
)

best_duration_peak = list(durations)[best_peak_index[0]]
best_start_peak = list(start_days)[best_peak_index[1]]
best_peak = peak_matrix[best_peak_index]

print("Best intervention for reducing peak cases:")
print("Duration:", best_duration_peak, "days")
print("Start day:", best_start_peak)
print("Peak cases:", best_peak)

best_total_index = np.unravel_index(
    np.argmin(total_matrix),
    total_matrix.shape
)

best_duration_total = list(durations)[best_total_index[0]]
best_start_total = list(start_days)[best_total_index[1]]
best_total = total_matrix[best_total_index]

print("\nBest intervention for reducing total cases:")
print("Duration:", best_duration_total, "days")
print("Start day:", best_start_total)
print("Total cases:", best_total)

weekly_cases_no_control, M_no_control = model(0, 1, 0)

baseline_peak = np.max(weekly_cases_no_control)

print("Baseline peak:", baseline_peak)

peak_reduction = baseline_peak - peak_matrix

peak_reduction_percent = (
    (baseline_peak - peak_matrix) / baseline_peak
) * 100
# Find day of maximum mosquito abundance without control
weekly_cases_no_control, M_no_control = model(0, 1, 0)

mosquito_peak_day = t[np.argmax(M_no_control)]

print("Mosquito peak day:", mosquito_peak_day)
...
plt.imshow(
    ...
    extent=[1, 365, 1, 42],
    vmin=0,
    vmax=np.max(peak_reduction_percent)
)
...
plt.colorbar(label="Reduction in peak dengue cases (%)")

plt.xlabel("Intervention start day")
plt.ylabel("Intervention duration (days)")
plt.title("Effect of larval control timing and duration on peak dengue cases")

plt.xticks(np.arange(0, 366, 25))
plt.yticks(np.arange(1, 43, 5))
