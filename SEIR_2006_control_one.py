...
results = pd.DataFrame({
    "Start day": list(start_days),
    "Peak weekly cases": peak_list,
    "Total annual cases": total_list
})

print(results)

best_total = results.loc[
    results["Total annual cases"].idxmin()
]

print(best_total)

best_peak = results.loc[
    results["Peak weekly cases"].idxmin()
]

print(best_peak)

plt.figure(figsize=(8,5))
plt.plot(start_days, peak_list)
plt.xlabel("Intervention start day")
plt.ylabel("Peak weekly cases")
plt.title("Effect of intervention timing")
plt.grid(True)
plt.show()

plt.figure(figsize=(8,5))
plt.plot(start_days, total_list)
plt.xlabel("Intervention start day")
plt.ylabel("Total annual infections")
plt.title("Effect of intervention timing")
plt.grid(True)
plt.show()
