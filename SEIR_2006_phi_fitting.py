...
weekly_I = model(beta0,epsilon,phi) #runs model using my initial guess

def objective_beta(scaled_beta):  #optimising beta
    beta = scaled_beta[0] * 1e-6
    weekly_I = model(beta,epsilon, phi)
...
def objective_epsilon(epsilon):
    ...
    weekly_I = model(beta0, epsilon[0], phi)
...
def objective_phi(phi):
    ...
    weekly_I = model(beta0, epsilon, phi[0])
    rss = np.sum((cases - weekly_I)**2)
    return rss
...
weekly_I = model(best_beta, best_epsilon,best_phi)
